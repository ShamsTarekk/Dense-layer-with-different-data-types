...
--        MEM_INIT_INPUTS : in work.my_types.mem_inputs64;
--        MEM_INIT_WEIGHTS : in work.my_types.mem_weights64;
--        MEM_INIT_BIASES : in work.my_types.mem_biases64;
--        clk       : in  std_logic;
--		rst_n     : in  std_logic;
--		start     : in  std_logic;
...
--architecture behavior of fsm_dense_fp64 is
...
--    type mo  is array (0 to 9, 0 to 63) of float64;
--    type mi1 is array (0 to 63) of float64;
--    type bi  is array (0 to 9)  of float64;
...
--        act6,act7,act8,act9,act10 : in float64;
...
--            );
--    end generate;
--add_req: getting_Address_of_maxfp64
--        port map(
--            clk => clk,
--            act1 => acc_output(0),
--            act2 => acc_output(1),
...
--        );
--process(clk, rst_n)
--            variable v_image_idx : integer range 0 to 999 := 0;
--        begin
--            if rst_n = '0' then
--                state <= S_IDLE;
--                out_valid_r <= '0';
...
--                                weight_buf(y,i) <=
--                                    to_float(to_2d64(MEM_INIT_WEIGHTS)(y,i),11, 52);
--                            end loop;
--                        end loop;
        
--                        for i in 0 to 63 loop
--                            input_buf(i) <=
--                                to_float(MEM_INIT_INPUTS(v_image_idx*64 + i),11, 52);
--                        end loop;
        
--                        for k in 0 to 9 loop
--                            bias_buf(k) <=
--                                to_float(MEM_INIT_BIASES(k),11, 52);
--                        end loop;
