...
N_NEURONS  : integer := 10
    ...
    port(
        ...
        MEM_INIT_INPUTS : in work.my_types.mem_inputs16;
        MEM_INIT_WEIGHTS : in work.my_types.mem_weights16;
        MEM_INIT_BIASES : in work.my_types.mem_biases16;
        clk       : in  std_logic;
		rst_n     : in  std_logic;
		start     : in  std_logic;
...
architecture behavior of fsm_dense_fp16 is
    ...
    -- FP16 arrays
    type mo  is array (0 to 9, 0 to 63) of float16;
    type mi1 is array (0 to 63) of float16;
    type bi  is array (0 to 9)  of float16;
...
component getting_Address_of_maxfp16 is
    Port (
        clk : in std_logic;
        act1,act2,act3,act4,act5,
        act6,act7,act8,act9,act10 : in float16;
...
add_req: getting_Address_of_maxfp16
        port map(
            clk => clk,
            act1 => acc_output(0),
            act2 => acc_output(1),
                    ...
                    when S_LOAD_FILES =>
                        for y in 0 to 9 loop
                            for i in 0 to 63 loop
                                weight_buf(y,i) <=
                                    to_float(to_2d16(MEM_INIT_WEIGHTS)(y,i), 5, 10);
                            end loop;
                        end loop;
        
                        for i in 0 to 63 loop
                            input_buf(i) <=
                                to_float(MEM_INIT_INPUTS(v_image_idx*64 + i), 5, 10);
                        end loop;
        
                        for k in 0 to 9 loop
                            bias_buf(k) <=
                                to_float(MEM_INIT_BIASES(k), 5, 10);
                        end loop;
