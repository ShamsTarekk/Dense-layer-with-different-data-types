...
port(
    MEM_INIT_INPUTS  : in work.my_types.mem_inputs16f;
    MEM_INIT_WEIGHTS : in work.my_types.mem_weights16f;
    MEM_INIT_BIASES  : in work.my_types.mem_biases16f;
    clk       : in  std_logic;
    rst_n     : in  std_logic;
    start     : in  std_logic;
...
architecture behavior of fsm_dense_fx16 is
    ...
    type mo  is array (0 to 9, 0 to 63) of fixed16;
    type mi1 is array (0 to 63) of fixed16;
    type bi  is array (0 to 9) of fixed16;
    ...
    -- Components
    component Accumulatorfixed16 is
        Port (
    ...
    component getting_Address_of_maxfixed16 is
        Port (
            clk : in std_logic;
            act1, act2, act3, act4, act5,
            act6, act7, act8, act9, act10 : in fixed16;
...
begin
    ...
    -- Max address
    ------------------------------------------------------------------------
    add_req: getting_Address_of_maxfixed16
        port map(
            clk   => clk,
            act1  => acc_output(0),
            act2  => acc_output(1),
                ...
                when S_LOAD_FILES =>
                    for y in 0 to 9 loop
                        for i in 0 to 63 loop
                            weight_buf(y,i) <= fixed16(MEM_INIT_WEIGHTS(y*10 + i));
                        end loop;
                    end loop;

                    for i in 0 to 63 loop
                        input_buf(i) <= fixed16(MEM_INIT_INPUTS(v_image_idx*64 + i));
                    end loop;

                    for k in 0 to 9 loop
                        bias_buf(k) <= fixed16(MEM_INIT_BIASES(k));
                    end loop;
